...
function OpenMenuGarage(garage, KindOfVehicle)
	...
	local elements = {
		{label = "Zwróć pojazd ("..Config.Price.."$)", value = 'return_vehicle'},
	}
...
function ListVehiclesMenu(garage, KindOfVehicle)
	...
	ESX.TriggerServerCallback('eden_garage:getVehicles', function(vehicles)
		if not table.empty(vehicles) then
			for _,v in pairs(vehicles) do
				...
				if(v.fourrieremecano)then
					labelvehicle = vehicleName..': Zewnętrzny kanał'
				elseif (v.state)  then
					labelvehicle = vehicleName..': Zwrócony'
				else
					labelvehicle = vehicleName..': Wyjście'
				end	
				table.insert(elements, {label =labelvehicle , value = v})
				
			end
		else
			table.insert(elements, {label ="Brak samochodów w garażu" , value = nil})
		end
		ESX.UI.Menu.Open(
		...
		function(data, menu)
			local elem = {}
			table.insert(elem, {label ="Wypuść samochód" , value = 'get_vehicle_out'})
			table.insert(elem, {label ="Zmień nazwę samochodu" , value = 'rename_vehicle'})
