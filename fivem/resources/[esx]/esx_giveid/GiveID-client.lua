...
AddEventHandler('esx_giveid:GiveIdResponse', function(response)
	if response then 
		ESX.ShowNotification("Dałeś swój dowód do ~y~" .. response)
	else
		ESX.ShowNotification("~r~Bład nie można dać dowodu")
	end
end)
...
function showIdNotification(msg, playerServerId)
	...
	DrawCTRPNotification('Kierowca', 'Identyfkator', msg, mugshotStr, 1)
